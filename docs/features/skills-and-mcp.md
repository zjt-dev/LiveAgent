...
桌面端和 WebUI 在「设置 → 资源与自动化」中统一提供 Skills、MCP、定时任务、记忆与 Hooks。Skills/MCP 复用完整的 Hub 管理界面，包括已安装资源与商店。

「设置 → 系统设置 → 侧栏快捷入口」可分别显示或隐藏 Skills、MCP、定时任务和记忆，默认全部显示。开关只影响入口可见性，不会停用资源、删除配置或停止定时任务；隐藏后仍可从设置进入管理页面。
