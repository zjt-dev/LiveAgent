...
| [design/remote-workspace-sidebar.md](design/remote-workspace-sidebar.md) | 活动项目是远程文件夹（`ssh://<hostId>/<abs>`）时的「远程工作空间」侧栏：Bash 与 SFTP 同屏、同一条 SSH 会话 | 远程工作空间与项目工具开发 |
| [design/conversation-workspace-follow.md](design/conversation-workspace-follow.md) | 打开会话时活动工作区跟随（侧栏高亮 / 列表 scope / 右栏项目上下文三者对齐） | 侧栏与工作区开发 |
